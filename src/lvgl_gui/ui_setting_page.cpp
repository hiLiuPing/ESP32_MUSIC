...
void redraw() { lv_label_set_text(title, editing ? "设置编辑" : "设置"); for (uint8_t i = 0; i < kItems; ++i) { char v[24] = {}; value(i, v, sizeof(v)); char row[96] = {}; std::snprintf(row, sizeof(row), "%c %-18s %s", navigation && i == selected ? '>' : ' ', labels[i], v); lv_label_set_text(rows[i], row); lv_obj_set_style_bg_color(rows[i], navigation && i == selected ? lv_color_black() : lv_color_white(), 0); lv_obj_set_style_text_color(rows[i], navigation && i == selected ? lv_color_white() : lv_color_black(), 0); } }
