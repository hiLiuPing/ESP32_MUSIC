...
void gui_init() {
    ...
    GuiPageDescriptor *pages[] = {&ui_boot_page_descriptor(), &ui_home_page_descriptor(),
                                  &ui_music_page_descriptor(), &ui_poetry_page_descriptor(),
                                  &ui_setting_page_descriptor()};
    for (GuiPageDescriptor *page : pages) {
        if (!gui_page_manager_register(page)) abort();
    }
    settings_app_init();
    ui_popups_init();
    if (!gui_page_manager_load(UiPage::Boot)) abort();
}
