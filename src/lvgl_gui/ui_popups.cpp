...
void show_poetry() { PoetryEntry entry = {}; if (!poetry_app_get_random(PoetryCollection::Song3000, &entry)) return; lv_label_set_text(poetry_title, entry.title); lv_label_set_text(poetry_body, entry.body); lv_obj_clear_flag(poetry, LV_OBJ_FLAG_HIDDEN); lv_obj_move_foreground(poetry); poetry_visible = true; poetry_started = millis(); }
}
...
void ui_popups_service(UiPage page) { ui_popups_init(); lv_obj_set_style_text_font(poetry_title, ui_heiti_font_get(18), 0); lv_obj_set_style_text_font(poetry_body, ui_heiti_font_get(18), 0); lv_obj_set_style_text_font(notice_title, ui_heiti_font_get(16), 0); lv_obj_set_style_text_font(notice_body, ui_heiti_font_get(16), 0); const uint32_t now = millis(); const AppSettings settings = settings_app_get(); if (page == UiPage::Boot) { ui_poetry_popup_dismiss(); ui_system_popup_dismiss_immediate(); return; } if (current.type == SystemNotifyType::Info && notice_started != 0 && now - notice_started > 4500U) ui_system_popup_dismiss_immediate(); if (poetry_visible && now - poetry_started > static_cast<uint32_t>(settings.poetry_duration_s) * 1000UL) ui_poetry_popup_dismiss(); if (page != UiPage::Home) ui_poetry_popup_dismiss(); if (page == UiPage::Home && settings.poetry_interval_min != 0 && !poetry_visible && notice_started == 0 && static_cast<int32_t>(now - next_poetry) >= 0) { show_poetry(); next_poetry = now + static_cast<uint32_t>(settings.poetry_interval_min) * 60000UL; } SystemNotifyMessage message = {}; if (notice_started == 0 && system_notify_try_receive(&message)) ui_system_popup_show(message); }
