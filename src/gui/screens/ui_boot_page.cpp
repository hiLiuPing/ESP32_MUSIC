...
uint8_t hardware_ready_count(EventBits_t bits) {
    return static_cast<uint8_t>(((bits & HW_EVENT_DISPLAY_READY) ? 1 : 0) +
                                 ((bits & HW_EVENT_LITTLEFS_READY) ? 1 : 0) +
                                 ((bits & HW_EVENT_SD_READY) ? 1 : 0) +
                                 ((bits & HW_EVENT_CODEC_READY) ? 1 : 0));
}

bool boot_can_exit(EventBits_t bits) {
    return (bits & (HW_EVENT_LITTLEFS_READY | HW_EVENT_INIT_DONE)) != 0;
}
...
void draw(egui_canvas_t *canvas) {
    ...
    char progress[32] = {};
    std::snprintf(progress, sizeof(progress), "INITIALIZING %u/4", completed);
    ...
    const int16_t status_x = 130;
    const int16_t status_y = 161;
    const int16_t status_step = 30;
    const EventBits_t status_bits[] = {HW_EVENT_DISPLAY_READY, HW_EVENT_LITTLEFS_READY,
                                       HW_EVENT_SD_READY, HW_EVENT_CODEC_READY};
...
bool service() {
    const EventBits_t bits = xEventGroupGetBits(HardwareEventGroup);
    if (((millis() - started_ms) >= BOOT_EXIT_START_MS) && boot_can_exit(bits)) {
