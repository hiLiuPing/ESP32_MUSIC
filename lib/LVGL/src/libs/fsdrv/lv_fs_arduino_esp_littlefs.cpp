...
static void fs_init(void)
{
    /* LittleFS is mounted once by bsp_littlefs_init(). Calling LittleFS.begin()
     * here can race or double-mount the partition while LVGL starts. */
}
...
static void * fs_open(lv_fs_drv_t * drv, const char * path, lv_fs_mode_t mode)
{
    ...
    if(!bsp_littlefs_lock(pdMS_TO_TICKS(100U))) {
        return NULL;
    }
